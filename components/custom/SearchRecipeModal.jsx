import { View, Text, Modal, TouchableOpacity, Image, ScrollView, TextInput, ActivityIndicator } from 'react-native'
...
const SearchRecipeModal = ({_searchMode, _toggleSearchMode, _title}) => {
...
const handleSearch = () => {
  ...
  return (
    <Modal visible={_searchMode} transparent={true} animationType="fade" className="flex-1">
    <View className="flex-1 p-4 bg-background-offwhite opacity-95">
      ...
      {loading ? (
                  <Text className='mt-[20]'>Searching...</Text> // Show "Searching..." while loading
                  ) : searchAttempted && results.length === 0 ? ( // Show only if search has been attempted
                      ...
                      {results.map((result, index) => (
                        <View key={index} style={{ position: 'relative', marginBottom: 20 }}>
                          ...
                          <View style={{ position: 'relative' }}>
                            ...
                            />
                           <HeartButton 
                              onPress={() => console.log(`Liked recipe: ${result.title}`)} 
                            />
                            </View>
                            <Text className="font-bold text-[16px] ml-4 my-auto flex-shrink">{result.title}</Text>
                          </TouchableOpacity>
